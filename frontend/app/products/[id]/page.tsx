...
// ✅ 시간 파싱 (string "HH:mm[:ss]" or [H,M,S])
function parseEndTime(timeData: any) {
    const now = new Date();

    // 없으면 오늘 23:59:59
    if (!timeData) return new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59);
    ...
    if (typeof timeData === 'string') {
        const parts = timeData.split(':').map((v) => Number(v));
        hours = parts[0] ?? 23;
        minutes = parts[1] ?? 59;
        seconds = parts[2] ?? 0;
    } else if (Array.isArray(timeData)) {
        hours = timeData[0] ?? 23;
        minutes = timeData[1] ?? 59;
        seconds = timeData[2] ?? 0;
    } else if (typeof timeData === 'object') {
        // {hour, minute, second} 형태까지 방어
        hours = timeData.hour ?? 23;
        minutes = timeData.minute ?? 59;
        seconds = timeData.second ?? 0;
    }
...
export default function ProductDetailPage() {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            ...
            try {
                ...
                // ✅ ProductDetail 타입엔 closeTime/closingTime이 없으니 (data as any)로만 접근
                // ✅ endTime이 존재하면 그걸 우선 사용, 없으면 store.closeTime 등 fallback
                const rawTime =
                    (data as any).endTime ||
                    (data as any).store?.closeTime ||
                    (data as any).store?.closingTime ||
                    (data as any).closeTime ||
                    (data as any).closingTime;
                ...
                const mappedProduct = {
                    ...
                    // ✅ ProductDetail에 location이 없을 수 있으니 any로만 받음 (없으면 거리 계산 스킵됨)
    ...
    // 3️⃣ 거리 계산
    useEffect(() => {
        if (!myLocation || !product?.location) return;
        ...
        if (loc && typeof loc === 'object') {
            if ('y' in loc && 'x' in loc) {
                // Kakao/XY 형태
                storeLat = Number(loc.y);
                storeLng = Number(loc.x);
            } else if ('coordinates' in loc && Array.isArray(loc.coordinates)) {
                // GeoJSON [lng, lat]
                storeLat = Number(loc.coordinates[1]);
                storeLng = Number(loc.coordinates[0]);
            } else if ('lat' in loc && 'lng' in loc) {
                storeLat = Number(loc.lat);
                storeLng = Number(loc.lng);
            }
        }
    ...
    const handleAddToCart = async () => {
        ...
        try {
            ...
            if (
                confirm(`${product.name} ${quantity}개를 장바구니에 담았습니다!\n장바구니로 이동하시겠습니까?`)
            ) {
                router.push('/cart');
            }
        } catch (error: any) {
    ...
    const handleBuyNow = async () => {
        ...
        if (!confirm('바로 구매 시 장바구니의 다른 상품은 제외하고 이 상품만 주문합니다. 진행하시겠습니까?'))
            return;
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div>
                        <div className="relative aspect-square rounded-2xl overflow-hidden bg-white shadow-sm">
                            ...
                            {isSaleProduct && (
                                <div
                                    className={`absolute bottom-4 left-4 backdrop-blur-sm px-4 py-2 rounded-lg flex items-center space-x-2 shadow-lg ${
                  ...
                  <span className="text-2xl text-gray-400 line-through">
                    {Math.round(product.originalPrice).toLocaleString()}원
                                    ...
                                    <Badge
                                        variant="sale"
                                        className="text-lg px-3 py-1 bg-red-500 text-white border-none"
                                    >
                                        {Math.round(product.discount).toLocaleString()}원 할인
                                    </Badge>
                                </div>
                            )}
                            <div className="text-4xl font-bold text-primary-600">{product.salePrice.toLocaleString()}원</div>
                        ...
                        <div>
                            ...
                            <div className="flex items-center space-x-4">
                                ...
                                <div className="flex-1 text-right">
                                    ...
                                    <p className="text-2xl font-bold text-gray-900">{(product.salePrice * quantity).toLocaleString()}원</p>
                                </div>
                            </div>
                        </div>
                                ...
                                {isSaleProduct && (
                                    <>
                                        ...
                                        <p className="text-sm text-gray-500">
                                            * 본 상품은 마감 임박 상품으로 환불이 어려울 수 있습니다.
                                            <br />* 매장 방문 수령 시간을 꼭 확인해주세요.
                                        </p>
                                    </>
                                )}
                            </div>
