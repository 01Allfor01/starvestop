...
export default function MySubscriptionDetailPage() {
    ...
    const userSubscriptionIdRaw = (params as any)?.id;
    const userSubscriptionId = Number(userSubscriptionIdRaw);
    ...
    useEffect(() => {
        if (!Number.isFinite(userSubscriptionId)) return;
        ...
        const fetchData = async () => {
            try {
                ...
                // ✅ distance는 백엔드 계산값 그대로 사용
                setSubscription({
                    ...subData,
                    days,
                    mealTime,
                    pickupSchedule: `${days.join('.')} ${mealTime}`,
                    image: storeData.imageUrl || subData.imageUrl || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c',
                    distance: typeof subData.distance === 'number' ? subData.distance : null,
                });
    ...
    if (!Number.isFinite(userSubscriptionId)) {
        return (
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div className="flex-1 flex flex-col justify-between">
                            <div>
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    {subscription.distance !== null && typeof subscription.distance === 'number' && subscription.distance >= 0 && (
                ...
                {new Date(userSubscription.expiresAt).toLocaleDateString('ko-KR')}
                        ...
                        <div className="flex items-start">
                            ...
                            <span className="font-medium text-secondary-600">{getNextPaymentDate(userSubscription.expiresAt)}</span>
                        </div>
                    </div>
                </Card>
