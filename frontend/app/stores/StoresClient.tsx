...
export default function StoresClient() {
    ...
    const initialCategory = categoryFromUrl ? reverseCategoryMap[categoryFromUrl] || '전체' : '전체';
    ...
    useEffect(() => {
        if (categoryFromUrl) setSelectedCategory(reverseCategoryMap[categoryFromUrl] || '전체');
    ...
    const handleCategoryChange = (category: string) => {
        ...
        if (category === '전체') router.push('/stores');
        else router.push(`/stores?category=${categoryMap[category]}`);
    };
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {stores.map((store) => (
                            <Link key={store.id} href={`/stores/${store.id}`}>
                                <Card hover padding="none" className="overflow-hidden h-full">
                                    ...
                                    <div className="p-4">
                                        <h3 className="font-semibold text-gray-900 mb-2 line-clamp-1">{store.name}</h3>
                        ...
                        {store.distance !== undefined && store.distance !== null ? formatDistance(store.distance) : '-'}
                      </span>
                                        </div>
                                    </div>
                                </Card>
